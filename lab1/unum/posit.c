...
double to_double(struct posit32 p) {
    ...
    if (fexp > 2046) {
        fexpbits = (uint64_t) 2046u << 53u;
        ffracbits = -1;
    } else if (fexp < 1) {
        fexpbits = 0;
        ffracbits = (uint64_t) (MSB | (p.fraction >> 1)) << (32u);
        ffracbits = ffracbits >> -fexpbits;
    } else {
        fexpbits = (uint64_t) (fexp & 0x7FF) << 53u;
        ffracbits = (uint64_t) p.fraction << 32u;
    }
    ...
    memory_unit.u = fexpbits | (memory_unit.u >> 11u);
    memory_unit.u = ((uint64_t) p.sign << 63u) | (memory_unit.u >> 1u);
...
struct posit32 to_posit32(double f) {
    ...
    memory_unit.d = f;
    result.sign = memory_unit.u >> 63u;
    result.exponent = ((memory_unit.u >> 52u) & 0x7FF) - bias;
    result.fraction = (memory_unit.u << 12u) >> 32u;
