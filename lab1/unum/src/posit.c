...
// takes sign, exp, fraction and returns ready posit
...
// TODO negative numbers handling
float posit2float(posit a) {
    ...
    val.bits |= ((exp + 127) << 23); // exp TODO check that no overwriting sign bit
    val.bits |= fraction >> 9;
    return val.input;
}
...
posit multiplication(posit a, posit b) {
    ...
    int sign_a = a.data & 0x80000000;
    int sign_b = b.data & 0x80000000;

    if (sign_a) a.data = - a.data;
    if (sign_b) b.data = - b.data;
    ...
    //long long fraction_a = a.data << (32 - frac_len_a);
    long long fraction_a = a.data & 0x03FFFFFF;
    ...
    long long fraction_b = b.data & 0x03FFFFFF;

    int new_sign = (sign_a) ^ (sign_b);
    ...
    if (new_fraction > (((long long)1 << (l)) - 1)) {
        l += 1;
        new_exp += 1;
        new_fraction &= 0x000FFFFFFFFFFFFF;
    }
    ...
    if (new_sign) result.data = -result.data;
