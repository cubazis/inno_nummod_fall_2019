...
unpacked _fromDouble(double f)
{
    ...
    un.f = f;
    up.neg = (bool) RIGHTSHIFT(un.u, 63u);
    up.exp = (int) ((RIGHTSHIFT(un.u, 52u)) & 0x7FFu) - bias;
    up.frac = (_uint32) (RIGHTSHIFT(LEFTSHIFT(un.u, 12u), 32u));

    if (up.exp == -bias) {
        // normalize
        up.exp -= CLZ(up.frac);
        up.frac = LEFTSHIFT(up.frac, CLZ(up.frac) + 1);
    }
...
posit _toPosit(unpacked up)
{
    ...
    int reg = FLOORDIV(up.exp, POW_2(ES));
    _uint32 rs = MAX(-reg + 1, reg + 2);
    _uint32 exp = up.exp - POW_2(ES) * reg;

    _uint32 regbits;
    if (reg < 0) {
        regbits = RIGHTSHIFT(MSB, -reg);
    } else {
        regbits = LEFTSHIFT(MASK, POSIT_SIZE - reg - 1);
    }
    _uint32 expbits = LEFTSHIFT(exp, POSIT_SIZE - ES);


    _uint32 tmp = RIGHTSHIFT(regbits, 1) | RIGHTSHIFT(expbits, 1 + rs) | RIGHTSHIFT(up.frac, 1 + rs + ES);
...
unpacked _fromPosit(posit p)
{
    ...
    int lz = CLZ(LEFTSHIFT(p.bits, 1));
    int lo = CLZ(LEFTSHIFT(~p.bits, 1) | 1);
    int rs = MIN(MAX(lz, lo) + 1, POSIT_SIZE - 1);

    int reg = (lz == 0 ? lo - 1 : -lz);
    int exp = RIGHTSHIFT(LEFTSHIFT(p.bits, 1 + rs), POSIT_SIZE - ES);

    res.exp = POW_2(ES) * reg + exp;
    res.frac = LEFTSHIFT(p.bits, 1 + rs + ES);
...
double toDouble(posit a)
{
    ...
    if (fexp > 2046) {
        fexpbits = LEFTSHIFT((_uint64)2046, 53);
        ffracbits = -1;
    } else if (fexp < 1) {
        fexpbits = 0;
        ffracbits = LEFTSHIFT((_uint64)(MSB | RIGHTSHIFT(up.frac, 1)), 64 - POSIT_SIZE);
        ffracbits = RIGHTSHIFT(ffracbits, -fexp);
    } else {
        fexpbits = LEFTSHIFT((_uint64)(fexp & 0x7FF), 53);
        ffracbits = LEFTSHIFT((_uint64)up.frac, 64 - POSIT_SIZE);
    }
    ...
    un.u = ffracbits;
    un.u = fexpbits | RIGHTSHIFT(un.u, 11);
    un.u = LEFTSHIFT((_uint64)up.neg, 63) | RIGHTSHIFT(un.u, 1);

    if (LEFTSHIFT(un.u, 1) == 0) {
        un.u++;
    }
...
unpacked _add(unpacked a, unpacked b, bool neg)
{
    ...
    if (a.exp > b.exp) {
        r.exp = a.exp;
        bfrac = RIGHTSHIFT(bfrac, a.exp - b.exp);
    } else {
        r.exp = b.exp;
        afrac = RIGHTSHIFT(afrac, b.exp - a.exp);
    }

    frac = afrac + bfrac;
    if (RIGHTSHIFT(frac, POSIT_SIZE) != 0) {
        r.exp++;
        frac = RIGHTSHIFT(frac, 1);
    }

    r.neg = neg;
    r.frac = LEFTSHIFT(frac, 1);
...
unpacked _sub(unpacked a, unpacked b, bool neg)
{
    ...
    r.neg = neg;
    r.exp -= CLZ(frac);
    r.frac = LEFTSHIFT(frac, CLZ(frac) + 1);
...
posit mul(posit ap, posit bp)
{
    ...
    _uint64 afrac = HIDDEN_BIT(a.frac);
    _uint64 bfrac = HIDDEN_BIT(b.frac);
    _uint32 frac = RIGHTSHIFT(afrac * bfrac, POSIT_SIZE);
