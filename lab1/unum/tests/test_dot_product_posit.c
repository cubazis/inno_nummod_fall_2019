...
void init_v1(struct vector *v, posit a) {
    ...
    v->x3 = power(fromDouble(10), subtract(a, fromDouble(1)));
    v->x4 = power(fromDouble(10), subtract(a, fromDouble(2)));
    v->x5 = fromDouble(3);
    v->x6 = negate(power(fromDouble(10), subtract(a, fromDouble(5))));
}
...
posit dot(struct vector *v1, struct vector *v2) {
    posit a = add(multiply(v1->x1, v2->x1), multiply(v1->x3, v2->x3));
    posit b = add(multiply(v1->x4, v2->x4), multiply(v1->x6, v2->x6));
    posit c = add(multiply(v1->x2, v2->x2), multiply(v1->x5, v2->x5));
...
void print_vec(struct vector *v) {
    printf(
            "{%lf, %lf, %lf, %lf, %lf, %lf}\n",
            toDouble(v->x1), toDouble(v->x2), toDouble(v->x3), toDouble(v->x4), toDouble(v->x5), toDouble(v->x6)
    );
}
...
START_TEST(test_a5)
    {
        ...
        for (int i = 0; i < 5; ++i) {
            init_v2(y, bs[i]);
            printf("%lf\n", toDouble(dot(x, y)));
            ck_assert(fabs(toDouble(subtract(res, dot(x, y)))) < toDouble(eps));
        }
    }
